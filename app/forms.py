...
class CommonForm(FlaskForm):
    ...
    user_height = DecimalField(
        'User height', validators=[InputRequired()])
    ...
    maximum_pcl_obstacles = DecimalField(
        'Max. PCL Obstacles', validators=[InputRequired()])  # Maximum number of "detected" obstacles in the PointCloud to describe to the user
    detection_msg_period = DecimalField(
        'Detection message period', validators=[InputRequired()])  # Time in seconds (NO ESTA SIENDO USADO AUN)
    detection_msg_period = DecimalField(
        'Detection message period', validators=[InputRequired()])  # Time in seconds (NO ESTA SIENDO USADO AUN)
